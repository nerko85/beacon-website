...
if(window.location.pathname != "/"){
    const paragraph = document.querySelectorAll('main.container p');

    $('.container p').each(function(){
        const $this = $(this)[0];
        const scroll = TweenMax.from($this, 1, {
            y:50, opacity:0
        });
        
        const scrollAnim = new TimelineMax().add(scroll);
...
sections.forEach((section, i) => {
    ...
    const tween1 = TweenMax.from(title, .5, {
        x: '-=100',
        opacity: 0,
        ease: Power2.easeIn
    }, 'slika');

    const tween2 = TweenMax.from(imgWrap, .5, {
        y: 20,
        opacity: 0
    }, '-=0.2');

    const tween3 = TweenMax.from(txtSect, .4, {
        scaleX: 0,
        ease: Power4.easeIn
    }, '-=1')

    const tween4 = TweenMax.from(txtWrap, .6, {
        opacity: 0,
        y: -20,
        delay:.2
    })
...
(function() {
    const services = [
      ...
      function contentLoad(e){
        ...
        // e.target.addClass('current').sibligns().removeClass('current');
        // e.toElement.addClass('current');
    ...
    //   links.forEach(link=>link.addEventListener('touchstart', contentLoad));
    ...
    $('.services-nav a').each(function(){
       ...
       function populate(index){
        ...
        const title = textContainer.childNodes[1];
         ...
         TweenMax.to(imgSect, 1, {opacity:0, onComplete:function(){
          img.setAttribute('src',current.img);
         }});

         TweenMax.from(imgSect, 1, {opacity:0, x:30, delay:1.4});
         
         
         TweenMax.to(textContainer,.7,{y:-20, opacity:0});
         ...
         TweenMax.to(textContainer,.7,   {y:0, opacity:1, delay:1.7});
    ...
    contactBtn.addEventListener('click',modal.openModal);
    contactBtn.addEventListener('touchstart',modal.openModal);
    modalClose.addEventListener('click', modal.closeModal)
    modalClose.addEventListener('touchstart', modal.closeModal)
    ...
    //     TweenMax.staggerTo('.mobile-item',.5,{opacity:1, y:0,ease:Power1.easeOut});
    // });
	...
	$('.modal_btn').on('click',function(e){
		...
		$.ajax({
			...
			type : 'POST',
			data : $this.closest('#modal_form').serialize(),
